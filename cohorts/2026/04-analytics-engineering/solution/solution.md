...
```bash
>> C >> then fail because of lacking of upstream dependencies
dbt run --select +int_trips_unioned >> A
dbt run --select int_trips_unioned+ >> D
dbt run --select +int_trips_unioned >> B
```
...
```bash
SELECT sum(total_monthly_trips) FROM `dbt-study-486006.dbt_lnguyen.fct_monthly_zone_revenue` 
where service_type = "Green"
and revenue_month = '2019-10-01 00:00:00'
...
#### Solution
1. Use Kestra to load data into data warehouse

  - 1.1. docker compose up -d

  - 1.2. Execute flows to setup gcp and extract data into warehouse
2. Build models by dbt
  ```bash
  dbt build
  ```
3. Query on BQ
